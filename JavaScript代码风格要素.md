...
1920年，由威廉·斯特伦克(William Strunk jr .)撰写的[英语写作手册:风格的要素(The Elements of Style)](https://www.amazon.cn/dp/B01M59NZUB/ref=cngwdyfloorv2_recs_0?pf_rd_p=7645736c-6759-4677-9dfb-2a3fd04770aa&pf_rd_s=desktop-2&pf_rd_t=36701&pf_rd_i=desktop&pf_rd_m=A1AJ19PSB66TGU&pf_rd_r=8ETX103G0HH940XG9RVK&pf_rd_r=8ETX103G0HH940XG9RVK&pf_rd_p=7645736c-6759-4677-9dfb-2a3fd04770aa)出版了，这本书为英文写作提供了经受住时间考验的经典的准绳。你可以在自己的编码风格中应用类似的标准来达到提高编码水平的目的。

下面只是一些准绳，但不是一成不变的法则。（There may be valid reasons to deviate from them if doing so clarifies the meaning of the code）如果能够很好的阐释代码的含义，那么我们也可以避开这些规则，但请特别小心并时刻反思。这些准绳是经受住了时间考验的，有充分的理由：它们通常是正确的。如果要避开这些规则，一定要有充足的理由，而不要单凭一时的兴趣或者个人的风格偏好。
...
6. 利用判断true值的方式来编写代码。（这句有点超前，是自己通读全文之后的理解 put statements and expressions in positive form）
7. 不同的技术方案利用不同的代码组织结构来实现。（use parallel construction on parallel concepts）

### 1.一个function只做一件事，让function成为代码组合的最小单元

> 软件开发的本质是“组合”。 我们通过组合模块，函数和数据结构来构建软件。理解如果编写以及组合方法是软件开发人员的基本技能。

模块是一个或多个function或数据结构的简单集合，我们用数据结构来表示程序状态，只有在函数执行之后，程序状态才会发生有趣的变化。
...
所有有效的应用程序都需要I/O，并且很多程序都遵循一定的程序执行顺序（procedural sequences）这种情况下，你的大部分函数应该是映射式函数：给定一些输入，返回相应的输出。
...
简洁的代码更清晰，因为它拥有更高的“信噪比”：读者必须筛选出较少“语法噪声”的代码来理解其含义。*更少的代码 = 更好的语法噪声 = 更强的代码含义信息传达*
...
第一种写法包含了不必要的语句。大括号，`function`关键字，以及`return`语句，这些对于熟悉箭头函数语法的开发者来说，没有任何意义。它存在的唯一意义只是让那些不熟悉ES6语法的开发者更好的理解代码。

ES6自2015年已经成为语言标准，是时候去学习它了。

#### 删除不必要的代码
有时候，我们试图为不必要的事物命名。问题是人类的大脑在工作中可用的记忆资源有限，每个名称都必须作为一个单独的变量存储，占据一个可用的工作中的“记忆插槽点”。

由于这个原因，有经验的开发者懂得删除不必要的变量。（原文为learn to，但根据上下文的理解，应该是“懂得”）

例如，大多数情况下，你应该省略仅仅用来当做返回值的变量。你的函数名应该已经提供了关于函数返回值明确的信息。看看下面的：
...
另一个开发者通常用来减少变量名的做法是，利用函数组合以及`point-free`风格。

`Point-free`风格是一种定义函数方式，定义成一种与参数无关的合成运算。实现`point-free`风格常用的方式包括库里化以及函数组合。
...
让我们来看另一个使用函数组合的例子。函数组合是将一个函数的运行结果传入另一函数并执行的过程。 也许你没有意识到，你一直在使用函数组合。 例如，当你链式调用`.map()`或`promise.then()`时，你就是在使用它。大多数情况下，看起来是这样的：f(g(x))。
...
当每次使用promise链式写法的时候，你就是在做这样的事情。

几乎每一个函数式编程类库都至少有两种实用的组合函数：从右到左依次运行的`compose()`；从左到右依次运行的`pipe()`。

Lodash将这两个函数命名为`compose()`以及`flow()`。可以这样引入：
```
import pipe from 'lodash/fp/flow';
pipe(g, f)(20); // 42
```

下面的代码做着同样的事情，但代码量并未增加太多：
```
...
当你利用“命令式”代码（imperative code）将功能以及中间变量拼凑在一起时，就像疯狂使用胶带和胶水将这些部分胡乱粘贴起来一样。

记住：
- 在不会改变或者模糊化其所表达的含义的前提下，如果能用更少的代码表达同样的意思，你应该这么做。
- 不会改变或者模糊化其所表达的含义的前提下，如果能用更少的变量表达相同的意思，你应该这么做。
...
命名最好直接反应其返回值:（Name predicts and booleans as if they are yes or no questions）
...
#### 事件处理
事件处理以及生命周期函数由于是限定符，就不适用动词形式这一规则；相比于“做什么”，它们主要是“什么时候做”。对于它们，可以“<什么时候去做>，<动作>”这样命名，朗朗上口。
...
第二个写法中，看起来我们正在尝试去触发一个事件，而不是对其作出回应。
...
更细致一点。因为这些方法所作用的对象（组件）都是内置的。在方法名中加入这些对象是多余的。想一想如果你直接调用这些方法时：`component.componentWillUpdate()`。这就好像在说，“吉米吉米会在晚餐吃牛排。”你没有必要听到同一个对象的名字两次。
- `component.beforeUpdate(doSomething)`优于`component.beforeComponentUpdate(doSomething)`

**函数混合**是指将方法作为属性添加到一个对象上面。方法将在同一个流中被依次调用-如同装配线。每一个函数混合将`instance`作为入参，并在其调用下一个方法之前，将一些东西加在它上面。
...
### 4.避免一连串结构松散的，不知所云的代码
开发人员经常将一系列事件串联在一个进程中：一组松散的、相关度不高的代码被设计依次运行。过多的程序代码是形成“意大利面条”代码的原因。

这些程序经常通过相同的形式重复调用，每一个单独看起来都很巧妙的，但也会意外地报错。例如，用户界面组件与几乎所有其他的用户界面组件共享相同的核心需求。 其关注点可以分解成生命周期阶段，并由单独的函数方法进行管理。
...
此方法的另一个问题就是你不能在没有数据的情况下，简单地计算要展示的数据并生成标签。如果你已经加载了数据怎么办？你可以结束没有必要继续下去的工作了。（What if you’ve already loaded the data? You end up doing work that you didn’t need to do in subsequent calls.）

分拆关注点也使得它们更容易进行测试。我喜欢对我的应用程序进行单元测试，并在写代码时展示每次代码改变之后的测试结果。但是，如果我们将渲染代码绑定到数据加载代码，我不能简单地将一些假数据传递给渲染代码进行测试。我必须从端到端测试整个组件。而这个过程中，由于浏览器加载，异步I/O请求等等会耗费时间。

上面的代码不能从单元测试测试中得到即时反馈。而分拆功能点允许你进行单独的单元测试，得到反馈。
...
### 5.功能相连的代码写在一起
许多框架以及boilerplates规定了程序文件组织的方法，其中文件按照技术类型分组。如果你正在构建一个小的计算器获取一个待办事宜的app，这样做是很好的。但是对于较大的项目，通过功能特性将文件分组在一起是更好的方法。
...
当你通过功能特性来将文件分组，你可以避免在文件列表上下滚动，之后查找编辑所需要的文件这种情况。
...
#### 缺失值的命名
有时候我们只关心一个变量是否缺失，因此如果通过判断true值的方式来命名，我们得用`!`操作符来否定它。这些情况下，尽量采用强false值含义的命名。`not`以及`!`操作符会创建弱的表达式。
...
（ When we fail to do so for the sake of familiarity, we talk down to readers of our code so that they can better understand it, like an adult speaking baby-talk to a toddler.）
投入资源去培训开发者是值得的，这样能让他们更快的去使用简洁的语法，库里化以及组合。如果因为更熟悉老的方式而不明白代码时，我们可以跟读者去解释，以使其更好的理解，就如果成人教导小孩一样。 

我们可以假设读者对于代码实现一窍不通，但不要假设读者是傻子，或者读者压根不了解这一编程语言。
